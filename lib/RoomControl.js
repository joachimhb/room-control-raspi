...
class RoomControl {
      ...
      for(const button of room.buttons || []) {
        ...
        this.buttons[id] = new Button({
          ...
          onOpen: async() => {
            ...
            await mqttClient.publish(buttonStatus(room.id, id), {value: 'opened'}, {retain: true});
  ...
  async updateFans() {
    const {room, logger, status, fansTrailing, mqttClient} = this;

    // console.log(JSON.stringify(status, null, 2))

    for(const fan of room.fans || []) {
      const location = `${room.label}/${fan.label}`;
      const humidity = _.get(status, ['humidity', 'value']);

      const minHumidityThreshold = _.get(status, ['fans', fan.id, 'minHumidityThreshold', 'value'], fan.minHumidityThreshold);
      const maxHumidityThreshold = _.get(status, ['fans', fan.id, 'maxHumidityThreshold', 'value'], fan.maxHumidityThreshold);

      const minRunTime   = _.get(status, ['fans', fan.id, 'minRunTime', 'value'], fan.minRunTime);
      const lightTimeout = _.get(status, ['fans', fan.id, 'lightTimeout', 'value'], fan.lightTimeout);
      const trailingTime = _.get(status, ['fans', fan.id, 'trailingTime', 'value'], fan.trailingTime);

      const control    = _.get(status, ['fans', fan.id, 'control', 'value'], 'manual');
      const speed      = _.get(status, ['fans', fan.id, 'speed', 'value'], 'off');
      const speedSince = _.get(status, ['fans', fan.id, 'speed', 'since'], new Date());
      ...
      for(const lightId of fan.triggerLights || []) {
        const lightValue = _.get(status, ['lights', lightId, 'value']);

        if(lightValue === 'on') {
          const onSince = _.get(status, ['lights', lightId, 'since']);
          ...
          const offSince = _.get(status, ['lights', lightId, 'since']);
